plugins {
    id("com.android.library")
    id("org.jetbrains.kotlin.android")
    `maven-publish`
}

android {
    namespace = "com.resvg.mobile.coil"
    compileSdk = 35

    defaultConfig {
        minSdk = 24
        missingDimensionStrategy("features", "full")
    }

    compileOptions {
        sourceCompatibility = JavaVersion.VERSION_17
        targetCompatibility = JavaVersion.VERSION_17
    }

    kotlinOptions {
        jvmTarget = "17"
    }

    publishing {
        singleVariant("release") {
            withSourcesJar()
        }
    }
}

dependencies {
    api(project(":resvg-mobile"))
    api("io.coil-kt.coil3:coil-core:3.3.0")
}

publishing {
    publications {
        create<MavenPublication>("release") {
            groupId = "com.resvg"
            artifactId = "resvg-mobile-coil"
            version = "0.1.0"
            afterEvaluate {
                from(components["release"])
            }
            pom {
                name.set("resvg-mobile-coil")
                description.set("Coil 3 SVG decoder backed by resvg-mobile")
                url.set("https://github.com/keremkusmezerbetsson/resvg-mobile")
                licenses {
                    license {
                        name.set("The Apache License, Version 2.0")
                        url.set("https://www.apache.org/licenses/LICENSE-2.0.txt")
                    }
                }
                developers {
                    developer {
                        id.set("keremkusmezer")
                        name.set("Kerem Kusmezer")
                    }
                }
                scm {
                    connection.set("scm:git:git://github.com/keremkusmezerbetsson/resvg-mobile.git")
                    developerConnection.set("scm:git:ssh://git@github.com/keremkusmezerbetsson/resvg-mobile.git")
                    url.set("https://github.com/keremkusmezerbetsson/resvg-mobile")
                }
            }
        }
    }
}
